...
import { SERVER_GATEWAY_URL, SOCKET_SERVER } from '../../../config';
...
function Paypage({ show, setShow, course }) {
    ...
    const sendMessage = async (nameCourse) => {
        // Khởi tạo đối tượng stompClientInstance
        const stompClientInstance = Stomp.over(new SockJS(`${SOCKET_SERVER}`));

        // Thiết lập kết nối
        stompClientInstance.connect({}, () => {
            // Gửi tin nhắn sau khi kết nối thành công
            stompClientInstance.send("/app/inform_your_buy", {}, nameCourse);
        }, (error) => {
    ...
    const fetchBuyCourse = async (amount) => {
        try {
            if (user_._jwt !== undefined) {
                const response = await axios.post(`${SERVER_GATEWAY_URL}/api/student/pay/buy.course`, {
                    price: amount
                    , course: course.id
        ...
        <div style={{ display: show ? '' : 'none' }} className='pay-body'>
            <div className='pay-main'>
                ...
                <div style={{ display: (base64String !== '' && !payed) ? '' : 'none' }} className='deposit-body'>
                    ...
                    <input onChange={(e) => setCode(e.target.value)} value={code} style={{
